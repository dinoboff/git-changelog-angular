 ...
 *
 * @typedef {{repo: {owner: string, project: string}, issue: number}} Issue
 * @typedef {{
 *          type: string,
 *          scope: string,
 *          body: string,
...
module.exports = exports = {
  ...
  create(options = {}) {
    return exports.context(options)
      .then(context => this.render(context, options));
  },
   ...
   */
  context(options = {}) {
    const {rev: revision, previous, next, identifier} = options;
   ...
   *
   * @param  {object} release Changelog details
   * @return {string|void}
   */
  inferVersion(release) {
    if (!release.previous) {
      return;
    }
    ...
    return version.inc(release);
  },

  /**
   * Render release.
   *
   * @param  {object} context   Release to render
   * @param  {object} [options] Query options
   * @return {string}
   */
