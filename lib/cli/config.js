...
module.exports = function (argv = process.argv.slice(2)) {
  ...
  return getRange(revList, {gitDir, client, anyTag}).then(({rev, previous = null, next = null}) => ({
    client,
    gitDir,
    rev,
    previous,
...
function getRange(revList, {gitDir, client, anyTag}) {
  const cmd = [
    ...
    '--default',
    'HEAD',
    ...revList.filter(rev => rev.trim().startsWith('-') === false)
  ];

  return client.array(...cmd, {map: hash => hash || null}).then(rev => {
    if (rev.length > 1) {
      return {rev};
    }

    return lastTagRange({gitDir, anyTag, rev: rev[0]});
  });
}
...
function lastTagRange(options) {
  ...
  return Promise.all([
    ...
    const {hash, version: previous} = last;

    return {next, previous, rev: [revision, `^${hash}`]};
  });
}
